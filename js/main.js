...
function registerPageTransition() {
  window.onload = () => {
    ...
    for (let i = 0; i < anchors.length; i++) {
      ...
      const isTarget = (anchor.attributes.getNamedItem("target") + "").substring(0, 4) !== "null";
      if (!isTarget) {
...
function initHamburgerMenu() {
  ...
  const title = document.querySelector(".main-nav__title");
...
function dragImages(theContainer, theClassName) {
  ...
  function drag(e) {
    if (active) {
      ...
      document.querySelector("#" + dragItem).style.transform = "translate3d(" + o.ox + "px, " + o.oy + "px, 0)";
    }
  }
}
